...
from ase.optimize import BFGS ( BFGS 구조 최적화 알고리즘, force를 줄여 구조 안정화 진행) --AI
...
from phonopy.structure.atoms import PhonopyAtoms ( ASE에서 읽은 원자 구조 파일을 phonopy 방식에 맞는 구조형식을 불러오는 방법)
...
def run_total_dos_safe(phonon, freq_min): (run_total_dos 명령어는 공식 기반, dos 계산 명령어)
    try:
...
EV_USE_SUPERCELL_TOTAL = False (e-v data > input cell 기준 사용, True면 supercell 기준 사용) --AI
...
def list_displaced_poscars(scale_dir: Path): (0.92 ~1.20까지 POSCAR 번호가 생성되는데 이때 번호를 순서대로 정렬)
...
def enforce_fc_asr_and_symmetry(phonon_obj): (force constant 값의 오차나 noise들을 대칭에 맞게 정리) --AI
    fc = phonon_obj.force_constants (계산된 force constant를 불러오기)
    if fc is None:
        raise RuntimeError("force_constants is None")

    try:
        from phonopy.harmonic.force_constants import (
            set_translational_invariance, (수치 오차, 구조변위로 인한 noise, ML potential 오차로 인한 0이어야 하는 값을 맞게끔 정리) --AI 
            set_permutation_symmetry,
        )
        set_translational_invariance(fc)
        set_permutation_symmetry(fc) (원자들 사이에서 미치는 영향에는 대칭 관계가 있는데 비대칭 > 대칭으로 바꾸어 정리) --AI
    except Exception:
        pass

    try:
        if hasattr(phonon_obj, "symmetrize_force_constants"):
            phonon_obj.symmetrize_force_constants() (추가 symmetry 보정, force constant 값을 보고 대칭에 맞게 정리)
    except Exception:
        pass
